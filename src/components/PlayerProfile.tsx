                    ...
                    return (
                      <div key={m.id} className={`flex items-center gap-3 py-2 px-3 rounded-lg border-l-4 ${win ? 'border-l-green-500 bg-green-50 dark:bg-green-900/10' : 'border-l-red-400 bg-red-50 dark:bg-red-900/10'}`}>
                        ...
                        <div className="text-xs text-gray-500 text-right flex-shrink-0">
                          {sets.map((s, i) => <span key={i} className="block">{s.length === 4 ? `${s[0]}–${s[1]}(${Math.min(s[2],s[3])})` : `${s[0]}–${s[1]}`}{s.length === 3 && s[2] === 1 ? <span className="text-xs text-orange-500 ml-0.5">Supertiebreak</span> : null}</span>)}
                        </div>
                      </div>
                    )
                  })}
